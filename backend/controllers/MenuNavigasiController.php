...
class MenuNavigasiController extends Controller
{
    ...
    public function actionIn($ids, $type)
    {
        switch ($type) {
            case 'delete':
                Yii::$app->db->createCommand("DELETE menu_navigasi WHERE id_menu IN($ids)")->execute();
                break;
            case 'active':
                Yii::$app->db->createCommand("UPDATE menu_navigasi SET status = 1 WHERE id_menu IN($ids)")->execute();
                break;
            case 'inactive':
                Yii::$app->db->createCommand("UPDATE menu_navigasi SET status = 0 WHERE id_menu IN($ids)")->execute();
                break;
            default:
                break;
        }
